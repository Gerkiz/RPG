...
--RPG Modules
require 'rpg.commands'
local ExplosiveBullets = require 'rpg.explosive_gun_bullets'
local RPG = require 'rpg.table'
local Functions = require 'rpg.functions'
local RPG_GUI = require 'rpg.gui'
    ...
    for i = 1, #players do
        ...
        if player and player.valid and not player.in_combat then
            if player.character and player.character.valid then
                ...
                rpg_t[player.index].mana = (round(rpg_t[player.index].mana * 10) / 10)
            end
        end
