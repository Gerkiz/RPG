...
local function grow_cell(cell)
    ...
    for i = 1, 4, 1 do
        ...
        if not this.explosives.cells[pos_to_key(position)] then
            local distance = math_sqrt((cell.origin_position.x - position.x) ^ 2 +
                (cell.origin_position.y - position.y) ^ 2)
...
local function on_entity_died(event)
    ...
    cell_birth(entity.surface.index, { x = entity.position.x, y = entity.position.y }, game.tick,
        { x = entity.position.x, y = entity.position.y }, final_damage)
end
...
function Public.detonate_chest(entity)
    ...
    cell_birth(entity.surface.index, { x = entity.position.x, y = entity.position.y }, game.tick,
        { x = entity.position.x, y = entity.position.y }, amount * damage)
    return true
end
...
function Public.detonate_entity(entity, amount, damage)
    ...
    cell_birth(entity.surface.index, { x = entity.position.x, y = entity.position.y }, game.tick,
        { x = entity.position.x, y = entity.position.y }, amount * damage, true)
    return true
end
