...
function Public.update_spell_gui(player, spell_index)
    ...
    if rpg_t.dropdown_select_index_1 == rpg_t.dropdown_select_index then
        local te1 = {
            '',
            { spell_1_data and spell_1_data.name and spell_1_data.name[1] or '---' },
            '\n',
    ...
    if rpg_t.dropdown_select_index_2 == rpg_t.dropdown_select_index then
        local te2 = {
            '',
            { spell_2_data and spell_2_data.name and spell_2_data.name[1] or '---' },
            '\n',
            shift_active_tooltip
        }
    ...
    if rpg_t.dropdown_select_index_3 == rpg_t.dropdown_select_index then
        local te3 = {
            '',
            { spell_3_data and spell_3_data.name and spell_3_data.name[1] or '---' },
            '\n',
            shift_active_tooltip
        }
...
function Public.extra_settings(player)
    ...
    local main_frame, inside_table = Gui.add_main_frame_with_toolbar(player, 'screen', settings_frame_name,
        settings_tooltip_name, nil, 'RPG Settings', true)
    if not main_frame then
        return
    end
    ...
    if rpg_extra.enable_crafting_chance then
        ...
        local style = enable_crafting_chance_label.style
        style.horizontally_stretchable = true
        style.height = 35
        style.vertical_align = 'center'

        local crafting_chance_flow = setting_grid.add({ type = 'flow' })
        local input_style = crafting_chance_flow.style
        input_style.height = 35
        input_style.vertical_align = 'center'
    ...
    if rpg_extra.enable_show_lvl_txt then
        ...
        local style = enable_show_lvl_txt_label.style
        style.horizontally_stretchable = true
        style.height = 35
        style.vertical_align = 'center'

        local show_lvl_txt_flow = setting_grid.add({ type = 'flow' })
        local input_style = show_lvl_txt_flow.style
        input_style.height = 35
        input_style.vertical_align = 'center'
    ...
    data.character_build_distance_bonus = create_bonus_label(player, setting_grid, ({ 'rpg_settings.build_dist_label' }),
        ({ 'rpg_settings.build_dist_tooltip' }), 'character_build_distance_bonus')
    data.character_crafting_speed_modifier = create_bonus_label(player, setting_grid,
        ({ 'rpg_settings.craft_speed_label' }), ({ 'rpg_settings.craft_speed_tooltip' }),
        'character_crafting_speed_modifier')
    data.character_health_bonus = create_bonus_label(player, setting_grid, ({ 'rpg_settings.health_bonus_label' }),
        ({ 'rpg_settings.health_bonus_tooltip' }), 'character_health_bonus')
    data.character_inventory_slots_bonus = create_bonus_label(player, setting_grid, ({ 'rpg_settings.inv_slots_label' }),
        ({ 'rpg_settings.inv_slots_tooltip' }), 'character_inventory_slots_bonus')
    data.character_item_drop_distance_bonus = create_bonus_label(player, setting_grid, ({ 'rpg_settings.drop_dist_label' }),
        ({ 'rpg_settings.drop_dist_tooltip' }), 'character_item_drop_distance_bonus')
    data.character_item_pickup_distance_bonus = create_bonus_label(player, setting_grid,
        ({ 'rpg_settings.reach_text_label' }), ({ 'rpg_settings.reach_text_tooltip' }),
        'character_item_pickup_distance_bonus')
    data.character_loot_pickup_distance_bonus = create_bonus_label(player, setting_grid,
        ({ 'rpg_settings.loot_pickup_label' }), ({ 'rpg_settings.loot_pickup_tooltip' }),
        'character_loot_pickup_distance_bonus')
    data.character_mining_speed_modifier = create_bonus_label(player, setting_grid, ({ 'rpg_settings.mining_speed_label' }),
        ({ 'rpg_settings.mining_speed_tooltip' }), 'character_mining_speed_modifier')
    data.character_reach_distance_bonus = create_bonus_label(player, setting_grid, ({ 'rpg_settings.char_reach_label' }),
        ({ 'rpg_settings.char_reach_tooltip' }), 'character_reach_distance_bonus')
    data.character_resource_reach_distance_bonus = create_bonus_label(player, setting_grid,
        ({ 'rpg_settings.resource_reach_label' }), ({ 'rpg_settings.resource_reach_tooltip' }),
        'character_resource_reach_distance_bonus')
    data.character_running_speed_modifier = create_bonus_label(player, setting_grid, ({ 'rpg_settings.mov_speed_label' }),
        ({ 'rpg_settings.mov_speed_tooltip' }), 'character_running_speed_modifier')
            ...
            mana_grid.add(
                {
                    ...
                    tooltip =
                    'Check the info button at upper right for more information\nOnly spells that you can cast are listed here.'
                }
            )
...
function Public.settings_tooltip(player)
    ...
    local main_frame, inside_table = Gui.add_main_frame_with_toolbar(player, 'center', settings_tooltip_frame, nil,
        close_settings_tooltip_frame, 'Spell info')
    if not main_frame then
        return
    end
    if not inside_table then
        ...
        for _, entity in pairs(spells) do
            if entity.enabled then
                ...
                if entity.type == 'item' then
                    local text = '[item=' ..
                    entity.entityName ..
                    '] ▪️ Level: [font=default-bold]' ..
                    entity.level ..
                    '[/font] Mana: [font=default-bold]' ..
                    entity.mana_cost .. '[/font].  Cooldown: [font=default-bold]' .. cooldown .. '[/font]'
        ...
        for _, entity in pairs(spells) do
            if entity.enabled then
                ...
                if entity.type == 'special' then
                    local text = '▪️ Level: [font=default-bold]' ..
                    entity.level ..
                    '[/font] Mana: [font=default-bold]' ..
                    entity.mana_cost .. '[/font]. Cooldown: [font=default-bold]' .. cooldown .. '[/font]'
